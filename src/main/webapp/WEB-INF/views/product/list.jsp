			...
			<div class="category-feed row">
				...
				<div class="category-feed__content col-12 col-md-9">
					<div class="commerce-category-header category-feed__content__header">
						...
						<div class="tap_set">
							<ul>
								<li <c:if test="${saleVO[0].deadlineInfo == false}">class="on"</c:if>>
									<a href="./list?deadlineInfo=0">진행중인 경매</a>
								</li>
								<li <c:if test="${saleVO[0].deadlineInfo == true}">class="on"</c:if>>
									<a href="./list?deadlineInfo=1"">마감된 경매</a>
								</li>
							</ul>
						</div>
						<div class="sub_tit">온라인경매</div>
						<div class="total_product">
							<strong>총 ${saleVO.size()}개</strong>
							<!-- 임시 -->
							<c:if test="${not saleVO[0].deadlineInfo}">
								<div class="order_by">
									<a
										href="./list?<c:if test="${param.categoryId != null}">categoryId=${param.categoryId}&</c:if>sc=1&list;"
										<c:if test="${classCheck == '1'}">class="on"</c:if>>등록순</a>
									<a
										href="./list?<c:if test="${param.categoryId != null}">categoryId=${param.categoryId}&</c:if>sc=2&list;"
										<c:if test="${classCheck == '2'}">class="on"</c:if>>참여높은순</a>
									<a
										href="./list?<c:if test="${param.categoryId != null}">categoryId=${param.categoryId}&</c:if>sc=3&list;"
										<c:if test="${classCheck == '3'}">class="on"</c:if>>임박순</a>
									<a
										href="./list?<c:if test="${param.categoryId != null}">categoryId=${param.categoryId}&</c:if>sc=4&list;"
										<c:if test="${classCheck == '4'}">class="on"</c:if>>낮은가격순</a>
									<a
										href="./list?<c:if test="${param.categoryId != null}">categoryId=${param.categoryId}&</c:if>sc=5&list;"
										<c:if test="${classCheck == '5'}">class="on"</c:if>>높은가격순</a>
								</div>
							</c:if>
						</div>
									...
									<c:when test="${classCheck == '2'}">
										<c:forEach items="${saleVO}" var="VO" varStatus="status" end="${saleVO.size()}">
											...
											<li>
												...
												<div class="imgbox">
													<a href="./detail?productId=${VO.productId}" class="img">
														<img src="/file/product/${requestScope[testVO].productFileVOs[0].fileName}" alt="">${requestScope[testVO].productName}</a>

													<div class="timedate" id="timertest"></div>
												</div>
												<div class="text">
													<c:if test="${not VO.deadlineInfo}">
														...
														<div class="pricebox">
															...
															<div>
																시작가 <span class="through"><fmt:formatNumber
																		value="${requestScope[testVO].productPrice}" pattern="###,###" /></span>
															</div>
															<div>
																현재가 <span class="price"><fmt:formatNumber value="${requestScope[bidAmountCheck]}"
																		pattern="###,###" /> <em style="font-size: 14px; vertical-align: top">↑</em></span>
															</div>
									...
									<c:when test="${classCheck == '3'}">
										<c:forEach items="${orderTime}" var="orderT">
											<c:forEach items="${saleVO}" var="VO" varStatus="status" end="${saleVO.size()}">
												...
												<c:set var="testVO" value="testVO${status.index}" />
												<c:set var="bidAmountCheck" value="bidAmountCheck${status.index}" />
												<c:set var="deadLineCompare"
													value="${requestScope[testVO].auctionPeriod*24*3600*1000+VO.productDate.getTime()}" />
									...
									<c:when test="${classCheck == '4' || classCheck == '5'}">
										...
										<c:forEach items="${saleVO}" var="VO" varStatus="status" end="${saleVO.size()}">
											...
											<li>
												...
												<div class="imgbox">
													<a href="./detail?productId=${VO.productId}" class="img">
														<img src="/file/product/${VO.productVOs[0].productFileVOs[0].fileName}" alt="">${VO.productVOs[0].productName}</a>
												...
												<div class="text">
													<c:if test="${not VO.deadlineInfo}">
														...
														<div class="pricebox">
															<div>
																시작가 <span class="through"><fmt:formatNumber value="${VO.productVOs[0].productPrice}"
																		pattern="###,###" /></span>
															</div>
															<div>
																현재가 <span class="price"><fmt:formatNumber value="${VO.maxBidAmount}" pattern="###,###" />
																	<em style="font-size: 14px; vertical-align: top">↑</em></span>
															</div>
															<a href="#" class="shophome">COBI</a>
														</div>
													</c:if>
